...
# Ensure token_size column exists
try:
    cursor.execute("ALTER TABLE sections ADD COLUMN token_size INTEGER")
    con.commit()
except sqlite3.OperationalError:
    pass  # Column already exists

# Create index on token_size for fast filtering
try:
    cursor.execute("CREATE INDEX token_size ON sections(token_size)")
    con.commit()
except sqlite3.OperationalError:
    pass  # Index already exists

# Step 1: Fill token_size column in parallel (only where NULL)
cursor.execute("SELECT id, content FROM sections WHERE token_size IS NULL")
...
if rows:
    ...
    print("No new rows found that need token_size update.")
    ...
    while True:
        ...
        for section_id, topic, content in batch:
            ...
            for chunk in chunks:
                ...
                cursor.execute(
                    "INSERT INTO sections (topic, content, token_size) VALUES (?, ?, ?)",
